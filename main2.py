...
parser.add_argument("--dataset", type=str, default='mimic', help="name of dataset")
...
if __name__ == '__main__':
    if opt.is_train == 0:
        ...
        if opt.dataset == '301':
           ...
           data_obj, data_min, data_max = tr_utils.get_data(f'{data_path}/mimic/vital_1.csv', opt, device)
           dim = 6
           dim_tcn = 7
           n_class = 6
    ...
    if opt.is_train == 1:
        modelg1, modelg2, modeld1 = tr_utils.load_model(f'./save_model/model_mimic_9.model', opt)
        modelg2.eval()
        modelg1.eval()
        device = torch.device(
            'cuda' if torch.cuda.is_available() else 'cpu')
